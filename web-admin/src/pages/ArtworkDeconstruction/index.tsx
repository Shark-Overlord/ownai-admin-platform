...
import { getArtworkDeconstruction, type ArtworkDeconstructionVO } from '../../api/artwork';
import './index.css';
...
export default function ArtworkDeconstruction() {
  ...
  const [dark, setDark] = useState(() => {
    ...
    return queryTheme ? queryTheme === 'dark' : localStorage.getItem('deconstruct_theme') !== 'light';
  });
  ...
  useEffect(() => {
    ...
    getArtworkDeconstruction(id).then((response) => setData(response.data)).catch((reason: unknown) => setError(reason instanceof Error ? reason.message : '解构数据加载失败')).finally(() => setLoading(false));
  }, [id]);
  ...
  const toggleTheme = () => setDark((current) => {
    localStorage.setItem('deconstruct_theme', current ? 'light' : 'dark');
    return !current;
  });
  ...
  if (loading || error || !data) return <div className={`dc-status-page${dark ? ' dark' : ''}`}><div className="dc-status-card">{loading ? <><span className="dc-spinner" />正在加载解构工作台…</> : <><strong>无法打开解构工作台</strong><p>{error || '作品不存在'}</p><a href="/artwork">返回作品管理</a></>}</div></div>;
