...
export default function CommunityManage() {
  ...
  async function edit(row?: PostRow) {
    ...
    try {
      ...
      setCurrent(post); form.resetFields(); form.setFieldsValue(post ? { ...post.draft, commentsEnabled: enabled(post.draft.commentsEnabled) } : { title: '', summary: '', markdown: '', tagIds: [], commentsEnabled: true });
  ...
  async function action(row: { id: string; version: number }, type: 'publish' | 'offline' | 'delete' | 'pin' | 'unpin') {
    setBusy(true);
    try {
      ...
      message.success(type === 'publish' ? '已发布保存的版本' : type === 'offline' ? '帖子已下线' : type === 'delete' ? '帖子已删除' : type === 'pin' ? '帖子已置顶' : '已取消置顶');
    } finally { setBusy(false); }
  }
  ...
  return <PageContainer title="新闻与帖子" subTitle="发布短篇图文和视频，按分类与标签组织内容">
    <Tabs onChange={() => void loadTerms().catch(() => {})} items={[
      { key: 'posts', label: '帖子列表', children: <ProTable<PostRow> actionRef={table} rowKey="id" scroll={{ x: 1150 }} params={{ sort }}
        ...
        columns={[
          { title: '标题', dataIndex: 'keyword', render: (_, row) => <Space direction="vertical" size={2}><Space size={4}><a onClick={() => void edit(row).catch(() => {})}>{row.title}</a>{enabled(row.pinned) && <Tag icon={<PushpinFilled />} color="gold">置顶</Tag>}</Space>{Number(row.hasUnpublishedChanges) === 1 && row.status === 'published' && <Tag color="orange">有未发布修改</Tag>}<span className="community-preview-kind">{row.previewMediaType === 'video' ? '视频预览' : row.previewMediaType === 'image' ? '图片预览' : '文字预览'}</span></Space> },
          ...
          { title: '操作', valueType: 'option', width: 360, render: (_, row) => <Space wrap size={2}>
            ...
            {row.status === 'published' && <><Popconfirm title="下线后将停止公开展示和互动。" onConfirm={() => action(row, 'offline')}><Button type="link" disabled={busy}>下线</Button></Popconfirm>
              <Button type="link" icon={enabled(row.pinned) ? <PushpinFilled /> : <PushpinOutlined />} disabled={busy} onClick={() => void action(row, enabled(row.pinned) ? 'unpin' : 'pin')}>{enabled(row.pinned) ? '取消置顶' : '置顶'}</Button>
    ...
    <Drawer rootClassName="community-drawer" title={current ? '编辑帖子' : '新建帖子'} width="min(1120px, 100vw)" open={open} onClose={close} maskClosable={false}
      ...
      <Form name="community-post-editor" form={form} layout="vertical" disabled={busy} onValuesChange={() => setDirty(true)}>
        ...
        <Tabs items={[
          { key: 'edit', label: '编辑与预览', children: <div className="community-editor"><Form.Item name="markdown" label="Markdown 正文"><Input.TextArea rows={22} maxLength={200000} className="community-source" /></Form.Item><section className="community-preview" aria-label="帖子草稿预览"><h2>{values?.title || '帖子预览'}</h2><CommunityMarkdown content={values?.markdown || ''} /></section></div> },
