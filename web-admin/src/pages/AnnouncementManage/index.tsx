...
import { Alert, Button, DatePicker, Drawer, Form, Input, InputNumber, Modal, Popconfirm, Select, Space, Switch, Tabs, Tag, Upload, message } from 'antd';
...
export default function AnnouncementManage() {
  ...
  const handleSave = async (values: any) => {
    ...
    try {
      const params = {
        ...
        popupEnabled: !!values.publicVisible && !!values.popupEnabled,
      };
      if (current) {
  ...
  return (
    <PageContainer title="系统公告" subTitle="普通通知与重要弹窗，独立于新闻帖子的讨论">
      <ProTable
        actionRef={actionRef}
        columns={columns}
        rowKey="id"
      ...
      >
        <Form name="announcement-editor" form={form} layout="vertical" onFinish={handleSave}>
          {current?.targetType === 'community_post' && <Alert type="info" showIcon style={{ marginBottom: 16 }} message={`关联帖子：${current.targetId}`} description="此公告来自已发布帖子，内容为生成时的摘要快照。用户前台接入关联跳转后可直接查看帖子；也可填写已上线的站内路径。" />}
          ...
          <Form.Item name="status" label="状态" rules={[{ required: true }]} extra={current?.status === 'published' ? '保存会更新已发布内容，不会重新触发已关闭的弹窗' : '先保存草稿，再从列表中发布'}>
            <Select options={statusOptions} disabled />
          </Form.Item>
          <Space align="start" size={32} wrap>
            <Form.Item name="publicVisible" label="公开展示" valuePropName="checked" extra="开启后，未登录访客也可查看">
              <Switch onChange={checked => { if (!checked) form.setFieldValue('popupEnabled', false); }} />
            </Form.Item>
            <Form.Item name="popupEnabled" label="弹窗提醒" valuePropName="checked" extra="每条关闭后不再提醒">
              <Switch disabled={!values.publicVisible} />
            </Form.Item>
          </Space>
          <Form.Item name="summary" label="更新摘要" dependencies={['popupEnabled']} rules={[
            ...
            <Input.TextArea rows={3} maxLength={300} showCount placeholder="概括这次更新带来的价值，用于列表和弹窗" />
          </Form.Item>
          <Form.Item name="actionLabel" label="按钮文字（选填）" dependencies={['actionPath']} rules={[
            { max: 30 }, ({ getFieldValue }) => ({ validator(_, value) { return getFieldValue('actionPath')?.trim() && !value?.trim() ? Promise.reject(new Error('请填写按钮文字')) : Promise.resolve(); } }),
          ]}>
            <Input maxLength={30} placeholder="例如：去体验、查看新教程" />
          </Form.Item>
          <Form.Item name="actionPath" label="按钮站内路径（选填）" dependencies={['actionLabel']} rules={[
            ({ getFieldValue }) => ({ validator(_, value) {
              if (!value?.trim()) return getFieldValue('actionLabel')?.trim() ? Promise.reject(new Error('请填写站内路径')) : Promise.resolve();
              try {
                const path = value.trim(); const decoded = decodeURIComponent(path.split(/[?#]/)[0]);
                if (!path.startsWith('/') || path.startsWith('//') || /[\\\s]/.test(path) || decoded.startsWith('//') || /[\\\x00-\x1f]/.test(decoded)) throw new Error();
                return Promise.resolve();
              } catch { return Promise.reject(new Error('请输入 /tutorials 这样的站内路径')); }
            } }),
          ]}>
            <Input maxLength={500} placeholder="例如：/tutorials 或 /pricing" />
          </Form.Item>
          ...
          <Form.Item name="content" label="图文正文（Markdown）" rules={[{ required: true, whitespace: true, message: '请输入公告内容' }]}>
            <Input.TextArea rows={12} placeholder={'## 本次更新\n\n介绍新功能、内容和使用方式。\n\n- 更新亮点一\n- 更新亮点二'} />
          </Form.Item>
          <Space wrap>
      ...
      <Modal open={preview !== null} title="发布预览" footer={null} zIndex={1200} width={preview === 'popup' ? 520 : 760} onCancel={() => setPreview(null)}>
        ...
        <div style={{ maxHeight: '65vh', overflowY: 'auto', padding: '16px 0' }} onClick={event => {
          ...
          <Space wrap style={{ marginTop: 24 }}>
            {values.actionLabel && values.actionPath && <Button type="primary">{values.actionLabel}</Button>}
            {preview === 'popup' && <Button type={values.actionLabel && values.actionPath ? 'default' : 'primary'}>查看详情</Button>}
          </Space>
        </div>
      </Modal>
    </PageContainer>
