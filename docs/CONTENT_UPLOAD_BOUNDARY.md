...
> 新接入的 Agent 和脚本请优先使用 [`CONTENT_API_V1.md`](CONTENT_API_V1.md) 中的统一内容接口。本文档保留原有上传规则和历史接口说明；旧密钥直写入口处于迁移兼容期，不能发布或更新已发布内容。

> **迁移提示：** 下文第 4～16 节记录的是旧调用方契约，不再作为新接入示例。新调用方只能把密钥放在 `X-Content-Asset-Key`，并通过 `/api/content/v1` 查询、上传和保存原模块草稿。待现有调用方迁移确认后，旧密钥写入口将正式关闭；管理员登录态接口不受影响。
...
以下 ID 是 2026-07-04 的历史示例，可能已经变化。实际调用必须通过统一接口 `GET /api/content/v1/taxonomy` 获取当前值，不能把本节 ID 写死在新脚本中。
