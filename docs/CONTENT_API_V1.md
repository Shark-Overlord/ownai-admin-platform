...
- 作品采用单记录生命周期。修改已发布作品时直接更新原作品 ID，并自动把状态改为下架；管理员确认后重新发布，不再创建替换草稿。
- 已发布的 Prompt、视频素材和教程文章仍会生成原生草稿副本。副本出现在对应模块原管理列表中；管理员可继续编辑并使用原发布按钮。发布时内容写回原资源 ID，草稿副本随后在同一事务中清理。
