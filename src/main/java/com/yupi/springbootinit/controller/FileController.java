...
public class FileController {
    ...
    private void validFile(MultipartFile multipartFile, FileUploadBizEnum fileUploadBizEnum) {
        ...
        if (FileUploadBizEnum.VIDEO_BACKGROUND_PREVIEW.equals(fileUploadBizEnum)) {
            validSuffix(fileSuffix, Arrays.asList("mp4", "webm"));
            if (fileSize > TWO_HUNDRED_M) {
                throw new BusinessException(ErrorCode.PARAMS_ERROR, "Preview video cannot exceed 200MB");
            }
            return;
        }
        if (FileUploadBizEnum.VIDEO_BACKGROUND_SOURCE.equals(fileUploadBizEnum)) {
            validSuffix(fileSuffix, Arrays.asList("mp4", "mov", "webm", "m4v"));
            if (fileSize > TWO_HUNDRED_M) {
                throw new BusinessException(ErrorCode.PARAMS_ERROR, "Original video cannot exceed 200MB");
            }
            return;
        }
